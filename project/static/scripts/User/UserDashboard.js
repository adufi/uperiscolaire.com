function readUserByID (id, onSuccess=undefined, onFailure=undefined) {
    return App.get(`/api/users/${id}/`, true)
        .then(data => {
            console.log(data)
...
function readSiblingByChild (id, onSuccess=undefined, onFailure=undefined) {
    return this._readSibling(`?child=${id}`, onSuccess, onFailure);
}

function readSiblingByParent (id, onSuccess=undefined, onFailure=undefined) {
    return this._readSibling(`?parent=${id}`, onSuccess, onFailure);
}

// Basic read request
function _readSibling (mod, onSuccess, onFailure) {
    return App.get(`/api/sibling/${mod}`, true)
        .then((data) => {
            console.log(data);
            if (data.hasOwnProperty('sibling')) {

                if (onSuccess)
                    onSuccess(data.sibling);
            }
        })
        .catch(err => {
            console.log(err);
...
// Basic read request
function readIntelsByParent (parent_id, onSuccess=undefined, onFailure=undefined) {
    return App.get(`/api/user/intel/?parent=${parent_id}`, true)
        .then((data) => {
            console.log(data);
            if (data.hasOwnProperty('intels')) {

                if (onSuccess)
                    onSuccess(data.intels);
            }
        })
        .catch(err => {
            if (onFailure)
...
function readClientByID(id, history=true, onSuccess=undefined, onFailure=undefined) {
    return App.get(`/api/clients/${id}/${(history) ? '?history=True' : ''}`, true)
    .then(data => {
        console.log(data);
...
function createIntel (payload, onSuccess=undefined, onFailure=undefined) {
    /**
     * @payload {
     *  
     * }
     */
    return App.post(`/api/intel/`, JSON.stringify(payload), true)
        .then(data => {
            if (data.hasOwnProperty('intels')) {
                if (onSuccess) onSuccess(data.intels);
            }
        })
        .catch(err => {
            if (onFailure)
...
function createClient (payload, onSuccess=undefined, onFailure=undefined) {
    return App.post(`/api/clients/`, JSON.stringify(payload), true)
        .then(data => {
            console.log(data);
            if (data.hasOwnProperty('client')) {
                if (onSuccess) onSuccess(data.client);
...
function updateUser (id, payload, onSuccess=undefined, onFailure=undefined) {
    return App.put(`/api/users/${id}/`, JSON.stringify(payload), true)
        .then(data => {
            if (data.hasOwnProperty('user')) {
...
function updateChild (id, payload, onSuccess=undefined, onFailure=undefined) {
    return App.put(`/api/children/${id}/`, JSON.stringify(payload), true)
        .then(data => {
...
function updateIntel (id, payload, onSuccess=undefined, onFailure=undefined) {
    return App.put(`/api/intel/${id}/`, JSON.stringify(payload), true)
        .then(data => {
            console.log(data);
            if (data.hasOwnProperty('intels')) {
                if (onSuccess) onSuccess(data.intels);
...
function updateClient (id, payload, onSuccess=undefined, onFailure=undefined) {
    return App.post(`/api/clients/${id}/`, JSON.stringify(payload), true)
        .then(data => {
            console.log(data);
            if (data.hasOwnProperty('client')) {
                if (onSuccess) onSuccess(data.client);
...
const VALIDATION = {
    ...
    number (value, not_null=false) {
        if (isNaN(value)) return 'Format incorrect.';
        if (typeof(value) !== 'number') return 'Format incorrect.';
        if (!value && not_null) return 'Ce champ est obligatoire.';
    ...
    /**
     * 
     * @param {str} value as text
     * @param {boolean} not_null|require
     */
    rawNumber (value, not_null=false) {
        // Not required
        // if (!not_null) {
        //     if (value)
        // }
    ...
    char (value, required=false, max_length=128) {
        if (!value) {
            if (required) {
    ...
    phone (value, required=false) {
        if (!value) {
            if(required) {
                return 'Le champ ne peux pas être vide';
            }
            return ''
        }
    ...
    select (value, object, required=false) {
        if (value === undefined && !required) {
            return '';
        }
        if (object.values.hasOwnProperty(value)) {
            return '';
        }
        return 'Choix incorrect.';
    },

    // https://www.w3resource.com/javascript/form/javascript-date-validation.php#:~:text=2.,yyyy%20is%20a%20valid%20year.
    dob_ (year, month, day, required=false) {
        if (!year && !month && !day) {
            if (required) {
                return 'Le champ ne peux pas être vide';
            }
        ...
        const dob_value = `${year}-${month}-${day}`;
        console.log(dob_value);

        var dateformat = /^\d{4}[\-](0?[1-9]|1[012])[\-](0?[1-9]|[12][0-9]|3[01])$/;
        // Match the date format through regular expression
        if(dob_value.match(dateformat)) {
            var dd = parseInt(day);
            ...
            // Every months except febuary
            if (mm === 1 || mm > 2) {
                if (dd > ListofDays[mm-1]) {
                    return 'Date incorrect';
                }
            }
            // Special case febuary
            if (mm === 2) {
                var lyear = false;
...
const FORMATING = {
    ...
    phone (value) {
        ...
        const _1 = value.substr(0, 4);
        const _2 = value.substr(4, 2);
        const _3 = value.substr(6, 2);
        const _4 = value.substr(8, 2);
        return `${_1} ${_2} ${_3} ${_4}`;
    }
};
...
const CREDITHISTORYTYPE = {
    values: {
        1: 'USER_ENTERED',
        2: 'ORDER_CANCELATION',
        3: 'TICKET_CANCELATION',
        4: 'CREDIT_CONSUMED',
        5: 'REFUND',
        6: 'SYSTEM_OP',
        7: 'MIGRATION',
    ...
    toSelect (value) {
        ...
        if (!_.hasOwnProperty(value)) return '<i>ERREUR TYPE</i>';
        return _[value];
    }
};
...
class User {
    ...
    createChild (onSuccess, onFailure) {
        ...
        payload['birthplace'] = this.birthplace;

        return App.post(`/api/user/`, JSON.stringify(payload), true)
            .then(data => {
                if (onSuccess)
                    onSuccess(data);
            })
    ...
    readUser (pk='', onSuccess=undefined, onFailure=undefined) {
        return App.get(`/api/user/${pk}/`, true)
            .then(data => {
                console.log(data)
                this._data = data;
    ...
    updateUser (onSuccess, onFailure) {
        ...
        const payload = {
            ...
            'phone_cell': this.phones.cell.replace(/ /g, ''),

            'address_1':    this.address.address1.capitalize(),
            'address_zip':  this.address.zip_code,
        };

        if (!payload.phone_cell) {
        ...
        // Optional fields
        payload['job']          = this.job.capitalize();

        payload['phone_pro']    = this.phones.pro.replace(/ /g, ''),
        payload['phone_home']   = this.phones.home.replace(/ /g, ''),

        payload['address_2']    = this.address.address2.capitalize(),
        ...
        <div class="col-md-6 col-sm-6 col-xs-12">
            <article class="Family-card">
                ...
                <div class="article-body ${((this.record_id) ? 'success' : 'danger')}">
                    <a href="/user/${this.id}/" class="child-name">${this.last_name} ${this.first_name}</a>
